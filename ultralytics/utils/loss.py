...
from ultralytics.utils.tal import RotatedTaskAlignedAssigner,NBCDAssigner,TaskAlignedAssigner, dist2bbox, dist2rbox, make_anchors
...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        # # iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)  #选择IOU
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, **{self.iou_type: True})
        loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum

        # 导入我们写的底层惩罚机制 WIoU
        # from ultralytics.utils.metrics import bbox_wiou_v3
        # iou = bbox_wiou_v3(pred_bboxes[fg_mask], target_bboxes[fg_mask])
        # loss_iou = ((iou) * weight).sum() / target_scores_sum

    
        # 导入我们写的底层惩罚机制 SNA-IoU
        # from ultralytics.utils.metrics import sn_alignment_loss

        # # 1. 直接计算融合了几何与动态权重的最终 Loss 项！
        # # 维度校验：内部处理全是 [N, 1]，安全！
        ...
        # # 2. 归一化输出
        # loss_iou = loss_item.sum() / target_scores_sum
...
# 增加一个类级别的 flag，确保整个类的生命周期内只打印一次状态
    _assigner_printed = False
