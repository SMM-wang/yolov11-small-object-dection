...
class NBCDAssigner(nn.Module):
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        # 核心替换：使用归一化巴氏距离 (NBCD) 替代 IoU
        overlaps[mask_gt] = self.nbcd_calculation(gt_boxes, pd_boxes)

        align_metric = bbox_scores.pow(self.alpha) * overlaps.pow(self.beta)
        return align_metric, overlaps

    def nbcd_calculation(self, gt_bboxes, pd_bboxes, eps=1e-4):
        """
        Calculate Normalized Bhattacharyya Distance (NBCD) based on 2D Gaussian Modeling.
        """
        gt_w = (gt_bboxes[..., 2] - gt_bboxes[..., 0]).clamp_(min=eps)
        gt_h = (gt_bboxes[..., 3] - gt_bboxes[..., 1]).clamp_(min=eps)
        gt_cx = gt_bboxes[..., 0] + gt_w / 2.0
        gt_cy = gt_bboxes[..., 1] + gt_h / 2.0

        # === 核心防震荡补丁：限制 pd_bboxes 的尺寸作弊 ===
        raw_pd_w = (pd_bboxes[..., 2] - pd_bboxes[..., 0]).clamp_(min=eps)
        raw_pd_h = (pd_bboxes[..., 3] - pd_bboxes[..., 1]).clamp_(min=eps)
        
        # 强制限制：如果预测框大得离谱，在算分布距离时，最多只按 GT 的 4 倍尺寸算方差。
        # 这不会改变实际预测出来的框，仅仅是在 TAL 分配打分时阻止巨大框刷高分。
        pd_w = torch.min(raw_pd_w, gt_w * 4.0) 
        pd_h = torch.min(raw_pd_h, gt_h * 4.0) 
        
        pd_cx = pd_bboxes[..., 0] + raw_pd_w / 2.0
        pd_cy = pd_bboxes[..., 1] + raw_pd_h / 2.0

        # 后续的高斯方差和距离计算完全保持原样不变
        var_gt_x = (gt_w / 2.0) ** 2
        var_gt_y = (gt_h / 2.0) ** 2
        var_pd_x = (pd_w / 2.0) ** 2
        var_pd_y = (pd_h / 2.0) ** 2

        sigma_x = (var_gt_x + var_pd_x) / 2.0
        sigma_y = (var_gt_y + var_pd_y) / 2.0

        term1 = 0.125 * (((gt_cx - pd_cx) ** 2) / sigma_x + ((gt_cy - pd_cy) ** 2) / sigma_y)
        
        det_sigma = sigma_x * sigma_y
        sqrt_det_gt_pd = (gt_w / 2.0) * (gt_h / 2.0) * (pd_w / 2.0) * (pd_h / 2.0)
        term2 = 0.5 * torch.log(det_sigma / sqrt_det_gt_pd.clamp_(min=eps))
