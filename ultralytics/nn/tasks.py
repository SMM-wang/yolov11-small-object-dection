...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            out_c = args[0]
            args = [in_channels_list, out_c]
            # c2 = [64, 128, 256]  # CFPT 输出 P3 P4 P5  n
            c2 = [64, 128, 256]

        elif m is SFPN:
            # args[0]: unified_channel, args[1]: num_sfbs
            # 我们需要自动推断 base_channels (输入通道数) 和 out_channels (输出通道数)

            # f 是来源层索引，例如 [4, 6, 10]
            if isinstance(f, list):
                # 从 ch 列表中获取对应层的输出通道数
                base_channels = [ch[x] for x in f]
            else:
                base_channels = [ch[f]]
            ...
            args = [base_channels, out_channels, *args]
            ...
            # args[0] 是 yaml 中写的通道数 (例如 128)
            c2 = args[0]

            # 应用模型宽度缩放 (width_multiple)
            # 例如对于 YOLO11n (width=0.25), 128 会变成 32
            if c2 != nc:
            ...
            c2 = c1[args[0]]  # 根据 level 索引获取当前层的通道数
            args = [c1, c2, *args]  # 重新封装参数

        elif m is My_Index:
            # Index 模块从列表/元组/字典中选择特定索引的特征
            idx = args[0]  # 获取索引参数

            # 如果输入通道是列表，则输出通道是列表中对应索引的值
            if isinstance(ch[f], list):
                c2 = ch[f][idx]  # 从列表中取出对应索引的通道数
            else:
                # 如果不是列表，可能是单个值，保持原样
                c2 = ch[f]
