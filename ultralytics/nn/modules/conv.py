...
class HWD(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # Haar变换会将通道数变为 4 倍 (LL, LH, HL, HH)
        # 所以我们需要一个 1x1 卷积将 4*in_channels 映射回 out_channels
    ...
    def forward(self, x):
        ...
        # 1. 切片操作 (类似于 SPD-Conv 或 Focus 层的切片)
        # 这里的切片是为了构造 2x2 的块
        x00 = x[..., 0::2, 0::2]  # 左上
        x01 = x[..., 0::2, 1::2]  # 右上
        x10 = x[..., 1::2, 0::2]  # 左下
        ...
        # 注意：这里我们没有除以2，因为后面有 BN 层会自动缩放，不除以2可以保留更强的梯度

        # 3. 通道拼接 (Channel Concatenation)
        # 拼接后形状: [B, 4*C, H/2, W/2]
        out = torch.cat([ll, lh, hl, hh], dim=1)
